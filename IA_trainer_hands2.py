...
while True:
    ...
    if len(lmList) != 0:
        if exercice_type == 1:
            ...
            back_per = np.interp(back_angle, (210, 310), (0, 100))
            back1_per = np.interp(back1_angle, (210, 310), (0, 100))
        ...
        if exercice_type == 2:
            ...
            rleg_per = np.interp(lright_angle, (190, 270), (0, 100))
            lleg_per = np.interp(lright_angle, (60, 170), (0, 100))

            back_angle = detector.findAngle(img, 12, 24, 26, True)
            back1_angle = detector.findAngle(img, 11, 23, 25, True)
            leg_bar = np.interp(lright_angle, (220, 310), (650, 100))
            cv2.rectangle(img, (1100, 100), (1175, 650), (0, 255, 0), 3)
            cv2.rectangle(img, (1100, int(leg_bar)),
                          (1175, 650), (255, 0, 0), cv2.FILLED)
            cv2.putText(img, f'{int(rleg_per)} %', (1100, 75), cv2.FONT_HERSHEY_PLAIN, 4,
                        (0, 0, 255), 4)
            # back_per = np.interp(back_angle, (210, 310), (0, 100))
            # back1_per = np.interp(back1_angle, (210, 310), (0, 100))
